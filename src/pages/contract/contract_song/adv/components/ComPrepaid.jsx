...
export const ComPrepaid = props => {
  ...
  const getData = () => {
    dispatch({
      type: 'contractSongList/fetchGetPrepaid',
      payload: {
        contract_song_id: pageId,
      },
    });
  }
        ...
        if (text == '2') {
          ...
          if (text) {
            for (let i = 0; i < row.specified_songs.length; i++) {
              ...
              if (i == sItem.length - 1) {
                renHtml.push(<span key={`spSong_${index}_${i}`}>{sItem[i].song_name} ({sItem[i].song_code})</span>);
              } else {
                renHtml.push(<span key={`spSong_${index}_${i}`}>{sItem[i].song_name} ({sItem[i].song_code})< br /></span>);
              }
            }
          }
      ...
      render: (text, row, index) => {
        ...
        return (
          (row.balance)
            ? ('')
            : (
              <Link to={`/contract/contract_song/prepaid/contract_id/${row.contract_id}/prepaid_id/${text}`}>
      ...
      <Card
        ...
        extra={
          <Button
            type="primary"
            onClick={() => {
              history.push(`/contract/contract_song/prepaid/contract_id/${pageId}`);
            }}
          >
            新增預付
          </Button>
...
export default connect(({ contractSongList, loading }) => ({
  contractSongList,
  loading: loading.effects['contractSongList/fetchGetPrepaid'],
}))(ComPrepaid);