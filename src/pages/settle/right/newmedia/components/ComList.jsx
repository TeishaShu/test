...
export const ComList = props => {
  ...
  const getData = (obj, isInit) => {
    let show_ui = {};
    // get sessionStorage
    const getSession = JSON.parse(sessionStorage.getItem('settle_right_newmedia_index'));
    ...
    sessionStorage.setItem('settle_right_newmedia_index', tempToString);
  ...
  const checkIsNowPhase = () => {
    if (settlePhaseList.newMediaRight && settlePhaseList.newMediaRight.current && settlePhaseList.newMediaRight.current.id && settlePhaseId == settlePhaseList.newMediaRight.current.id) {
      return true;
    }
        ...
        return (
          <Tooltip title="比對歌曲">
            <PauseCircleOutlined
              ...
              onClick={() => {
                if (checkUse) {
                  setCurrentStep({ step: 1, list: { ...row } });
                }
              }}
            />
          </Tooltip>
